...
// Search Bug Fixed + Cache Nuke Active
// ============================================================
...
// --- SEARCH ENGINE COMPLETELY REBUILT ---
...
window.performSearch = async (query) => {
    ...
    for (let i = 0; i < window.INVIDIOUS.length; i++) {
        // Drop the loop instantly if user typed a new letter
        if (currentId !== window.searchGlobalId) return null; 

        const base = window.INVIDIOUS[(window.invIdx + i) % window.INVIDIOUS.length];
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 7000); // Wait 7 seconds max
        
        try {
            // REMOVED "audio" tag to fix queries like "Mask". REMOVED Region locks.
            ...
            // REMOVED STRICT LENGTH LIMITS FOR MANUAL SEARCH! Let the user see everything.
...
window.bindSearch = () => {
    ...
    // Destroy duplicate event listeners from tab switching
    ...
    newElement.addEventListener('input', (e) => {
        ...
        // Wait 800ms to stop typing before hammering the API to prevent Too Many Requests (429) errors
        window.activeSearchTimer = setTimeout(async () => {
            ...
            // SECURITY CHECK: If user typed another letter while waiting, drop these old results!
