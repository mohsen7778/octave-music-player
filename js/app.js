...
// Absolute Override Active — Zero-Freeze Deployment
// ============================================================

// 1. ABSOLUTE OVERRIDE: Forcefully kill the splash screen instantly on script load
(function() {
    const clearSplash = () => {
    ...
    input.oninput = (e) => {
        ...
        window.activeSearchTimer = setTimeout(async () => {
            ...
            results.slice(0, 15).forEach(track => {
                ...
                el.innerHTML = `<img src="${window.getSafeThumb(track)}" style="width: 48px; height: 48px; border-radius: 6px; object-fit: cover; cursor: pointer;"><div class="list-info" style="cursor: pointer;"><div class="list-title">${window.escapeHTML(track.title)}</div><div class="list-subtitle">${window.escapeHTML(track.author)}</div></d